...
### 2.3 权限模型、角色矩阵与数据隔离设计

系统对医院模块采用**双层鉴权**，两者互补、缺一不可：

1. **操作级（菜单/按钮可见性与可点击性）**：基于 yudao 的 `@PreAuthorize("@ss.hasPermission('hospital:X:op')")` 注解 + 前端 `v-hasPermi` 指令。角色只有被授予对应权限码，相关菜单/按钮才会出现、才可点击。
2. **数据级（同一接口返回"全部"还是"仅自己"）**：`HospitalSecurityContext` 在 Service 层判定当前登录身份。其 `isAdmin()` **不再依赖 `LoginUser.getUserType()`**——所有后台账号（管理员/医生/患者）登录后该字段都固定为 `ADMIN=2`，旧实现会让所有角色都被误判为管理员，导致"仅自己"隔离完全失效——改为**按业务档案判定**："既未关联 `hospital_doctor` 档案、也未关联 `hospital_patient` 档案"的后台账号才视为管理员，否则按医生/患者身份做行级隔离。

> ⚠️ 关键陷阱：早期 `isAdmin()` 用 `getUserType() <= 2` 判定，因后台账号 userType 恒为 2，使医生/患者的数据隔离全部失效。

#### 2.3.1 角色-权限矩阵（已实现）

| 模块 | 管理员 | 医生 | 患者 |
|------|--------|------|------|
| 科室 / 医生 / 病人 / 病房 / 床位 / 药品 | 全部读写 | 仅查询（写按钮不可见） | 菜单不可见 |
| 就诊管理 | 读写 | 读写（仅自己） | 查询（仅自己） |
| 处方管理 | 读写 | 读写 + 发药（仅自己） | 查询（仅自己） |
| 收费管理 | 读写 + 支付 | 仅查询 | 查询 + 支付（仅自己） |
| 数据统计 | 可见 | **不可见（已从菜单移除）** | 不可见 |
| 知识图谱（疾病字典 / 症状字典 / 关联管理 / 图谱可视化） | 全部读写 | 仅查询（只读） | 仅图谱可视化 + 疾病字典只读 |
| AI 辅助诊断 | 可用 | **不可见（不授予该权限）** | 可用（核心患者功能） |
| AI 处方审核 | 可用 | 可用（仅审本人开具的处方） | **不可见（不授予该权限）** |
| 数据看板（5 个可视化页面） | 可见 | 可见 | **不可见（不授予该权限）** |

> **扩展模块角色分配要点**：患者角色（role_id=4）刻意只授予 `hospital:ai:assist-diagnosis`、`hospital:knowledge-graph:query`、`hospital:disease:query`——即「患者只能使用 AI 辅助诊断」，不能看到 AI 处方审核与数据看板；医生角色（role_id=3）授予知识图谱只读 + `hospital:ai:prescription-review` + `hospital:visual:query`，刻意**不授予** `hospital:ai:assist-diagnosis`。授权由 `ruoyi-vue-pro/sql/mysql/hospital_role_ext_grant.sql` 以幂等方式写入 `system_role_menu`（`tenant_id` 必须显式 =1；执行后必须 `redis-cli flushdb` 清权限缓存，否则新授权不生效）。
>
> **AI 处方审核的归属校验**：`AiServiceImpl.prescriptionReview` 在查出处方后追加身份判断——非管理员且关联医生档案的账号，只能审核 `doctor_id` 等于当前登录医生的处方，否则抛 `HOSPITAL_PERMISSION_DENIED`；患者因不持有 `hospital:ai:prescription-review` 权限码，会先在 `@PreAuthorize` 层被拦截，无需进入业务方法。

#### 2.3.2 几处刻意的设计取舍

- **故意不授予患者 `hospital:patient:query`**：`PatientRespVO` 包含身份证号/电话/住址/医保卡号/既往病史等隐私字段，一旦授权患者即可通过 `/hospital/patient/page` 看到全部患者信息。前端在缺该权限时直接跳过"患者"下拉、隐藏"患者"列，既无 403 弹窗也不泄露隐私。
- **对医生隐藏"数据统计"菜单**：通过移除 `system_role_menu` 中 `role_id=3` 对菜单 `5190`/`5191` 的授权，后端 `getRouters` 不再下发，医生侧边栏即不显示该入口。
- **写接口身份绑定（防越权）**：`Visit`/`Prescription` 的 `create` 强制将记录归属到当前登录医生/患者 id；`Patient.createPatient` 强制 `requireAdmin()`（与 `createDoctor` 一致，避免匿名建档）；`Bill.payBill` 校验账单归属当前患者，杜绝冒用他人 id。
- **分页查询隔离统一抽象**：`HospitalSecurityContext` 提供 `resolvePatientScope()` / `resolveDoctorScope()`（管理员或身份未知时返回 `null`＝可见全部数据），`Visit`/`Prescription`/`Bill`/`Patient` 的 `getXxxPage` 统一调用，消除四处重复的 `isAdmin()`+`getCurrentXxxId()` 分支。
- **统计接口严格 admin-only**：`StatsServiceImpl` 的 `getVisitTrend`/`getWardUsage`/`getMedicineStock` 对非管理员**抛 `HOSPITAL_DATA_ACCESS_DENIED`**（旧版静默返回空列表），数据隔离失败即显式暴露而非静默漏数据。
- **前端写按钮统一 `v-hasPermi` 守卫**：9 个业务页面的「新增/编辑/删除」及处方「发药」、床位「分配/释放」全部按权限显隐，从根上杜绝"点了才弹 403"。

#### 2.3.3 权限码完整性

已用脚本比对「医院模块全部 Controller 的 `@PreAuthorize` 所需权限码」与「菜单表实际存在的码」，确认除下表已补齐的 3 个缺失码外，模块内无其它遗漏，不存在隐性 403 源：

| 权限码 | 菜单 id | 说明 | 授予角色 |
|--------|---------|------|----------|
| `hospital:bill:pay` | 5185 | 账单支付（原缺失→支付一直 403） | 患者 |
| `hospital:prescription:dispense` | 5175 | 处方发药（原缺失→医生点发药 403） | 医生 |
| `hospital:bed:assign` | 5146 | 床位分配（原缺失→医生 403） | 医生 |
| `hospital:bed:release` | 5147 | 床位释放（原缺失→医生 403） | 医生 |

此外，为消除"打开页面即弹 403"，医生补 `doctor:query`(5111)、患者补 `doctor:query`(5111)/`department:query`(5102)/`medicine:query`(5161)（均为 `type=3` 查询按钮，只给接口权限、不新增菜单）。以上全部由 `ruoyi-vue-pro/sql/hospital-permissions-fix.sql` 以幂等方式补齐，详见《启动与测试指南》§4.1.2 的执行说明。
...
> 以下为 v1.0 的 10 张业务表建表语句，已在 `sql/opengauss/hospital.sql`（OpenGauss）和 `sql/mysql/hospital.sql`（MySQL）中实现。
...
> 上述守卫均有对应的 Mockito 单元测试覆盖（并发守卫相关共 16 个用例，位于 `ruoyi-vue-pro/yudao-module-hospital/src/test`），涵盖并发双占被拒、重复支付或发药被拒、终态不可改、引用存在时删药被拒等场景；医院模块单元测试现已扩至 33 个、覆盖全部 10 个业务模块。
